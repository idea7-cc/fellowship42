...
function assertUniqueLessonIds(lessons) {
...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            ...
            .unique();
        if (existing) {
            throw new Error(`A course with slug "${args.slug}" already exists in this church`);
        }
...
export const update = mutation({
    args: {
        ...
        courseType: v.optional(v.union(v.literal("new-member"), v.literal("volunteer-training"), v.literal("discipleship"), v.literal("leadership"), v.literal("bible-study"), v.literal("curriculum"))),
        deliveryMode: v.optional(v.union(v.literal("self-paced"), v.literal("group-led"), v.literal("cohort"), v.literal("hybrid"))),
    ...
    handler: async (ctx, { courseId, ...fields }) => {
        ...
        if (fields.ministryId) {
            await requireChurchScopedDocument(ctx, fields.ministryId, course.churchId, "Ministry");
        }
        if (fields.lessons) {
        ...
        if (fields.slug && fields.slug !== course.slug) {
            const existing = await ctx.db
                .query("courses")
                .withIndex("by_church_and_slug", (q) => q.eq("churchId", course.churchId).eq("slug", fields.slug))
                .unique();
            if (existing) {
                throw new Error(`A course with slug "${fields.slug}" already exists in this church`);
            }
        }
        const patch = {};
