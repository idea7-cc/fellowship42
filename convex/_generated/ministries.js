...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            ...
            .unique();
        if (existing) {
            throw new Error(`A ministry with slug "${args.slug}" already exists in this church`);
        }
...
export const update = mutation({
    ...
    handler: async (ctx, { ministryId, ...fields }) => {
        ...
        if (fields.slug && fields.slug !== ministry.slug) {
            const existing = await ctx.db
                .query("ministries")
                .withIndex("by_church_and_slug", (q) => q.eq("churchId", ministry.churchId).eq("slug", fields.slug))
                .unique();
            if (existing) {
                throw new Error(`A ministry with slug "${fields.slug}" already exists in this church`);
            }
        }
        const patch = {};
