...
export const listByChurch = query({
    ...
    handler: async (ctx, { churchId }) => {
        if (await hasChurchAccess(ctx, churchId)) {
            return await ctx.db
                .query("sermons")
                .withIndex("by_church_and_preached_at", (q) => q.eq("churchId", churchId))
                .order("desc")
                .collect();
        }
...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            ...
            .unique();
        if (existing) {
            throw new Error(`A sermon with slug "${args.slug}" already exists in this church`);
        }
...
export const update = mutation({
    ...
    handler: async (ctx, { sermonId, ...fields }) => {
        ...
        if (fields.slug && fields.slug !== sermon.slug) {
            const existing = await ctx.db
                .query("sermons")
                .withIndex("by_church_and_slug", (q) => q.eq("churchId", sermon.churchId).eq("slug", fields.slug))
                .unique();
            if (existing) {
                throw new Error(`A sermon with slug "${fields.slug}" already exists in this church`);
            }
        }
        const patch = {};
