...
export const listByChurch = query({
    ...
    handler: async (ctx, { churchId }) => {
        ...
        return await ctx.db
            .query("groups")
            .withIndex("by_church_and_status", (q) => q.eq("churchId", churchId).eq("status", "published"))
            .collect();
    },
});
/**
...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.ministryId) {
            await requireChurchScopedDocument(ctx, args.ministryId, args.churchId, "Ministry");
        }
        await Promise.all(args.leaderIds.map((leaderId) => requireChurchScopedDocument(ctx, leaderId, args.churchId, "Leader")));
        const existing = await ctx.db
            .query("groups")
            .withIndex("by_church_and_slug", (q) => q.eq("churchId", args.churchId).eq("slug", args.slug))
            .unique();
        if (existing) {
            throw new Error(`A group with slug "${args.slug}" already exists in this church`);
        }
...
export const update = mutation({
    args: {
        ...
        groupType: v.optional(v.union(v.literal("small-group"), v.literal("sunday-school"), v.literal("bible-study"), v.literal("support-group"), v.literal("serving-team"), v.literal("training-cohort"))),
    ...
    handler: async (ctx, { groupId, ...fields }) => {
        ...
        if (fields.ministryId) {
            await requireChurchScopedDocument(ctx, fields.ministryId, group.churchId, "Ministry");
        }
        if (fields.leaderIds) {
            await Promise.all(fields.leaderIds.map((leaderId) => requireChurchScopedDocument(ctx, leaderId, group.churchId, "Leader")));
        }
        if (fields.slug && fields.slug !== group.slug) {
            const existing = await ctx.db
                .query("groups")
                .withIndex("by_church_and_slug", (q) => q.eq("churchId", group.churchId).eq("slug", fields.slug))
                .unique();
            if (existing) {
                throw new Error(`A group with slug "${fields.slug}" already exists in this church`);
            }
        }
        const patch = {};
