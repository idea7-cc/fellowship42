    ...
    handler: async (ctx, { churchId }) => {
        ...
        if (await hasChurchAccess(ctx, churchId)) {
            const events = await ctx.db
                .query("events")
                .withIndex("by_church_and_start_date", (q) => q.eq("churchId", churchId))
                .order("asc")
                .collect();
        ...
        const allPublished = await ctx.db
            ...
            .order("asc")
            .collect();
        return allPublished.filter((event) => event.status === "published" && event.startDate >= now);
    },
});
/**
...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            ...
            .unique();
        if (existing) {
            throw new Error(`An event with slug "${args.slug}" already exists in this church`);
        }
...
export const update = mutation({
    ...
    handler: async (ctx, { eventId, ...fields }) => {
        ...
        if (fields.slug && fields.slug !== event.slug) {
            const existing = await ctx.db
                .query("events")
                .withIndex("by_church_and_slug", (q) => q.eq("churchId", event.churchId).eq("slug", fields.slug))
                .unique();
            if (existing) {
                throw new Error(`An event with slug "${fields.slug}" already exists in this church`);
            }
        }
        const patch = {};
