...
After a restore, keep the instance closed to traffic and use the
[agent credential reset command](agent-operator.md#reset-after-restore). It verifies
the portable identity before clearing disposable staff and agent credentials.
Passkeys remain usable only on the same church hostname. A changed hostname
