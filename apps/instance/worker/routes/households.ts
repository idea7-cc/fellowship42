...
import { z } from 'zod'
import type { Household, HouseholdMember } from '../../contracts/api'
  ...
  .superRefine((value, context) => {
    ...
    if (new Set(ids).size !== ids.length) {
      context.addIssue({
        code: 'custom',
        path: ['members'],
        message: 'Household members must be unique',
      })
    }
    if (value.members.filter((member) => member.isPrimary).length > 1) {
      context.addIssue({
        code: 'custom',
        path: ['members'],
        message: 'Only one member can be primary',
      })
    }
  })
...
async function findHousehold(
  ...
  const row = await db
    ...
    .first<HouseholdRow>()
  if (!row)
    throw new AppError(404, 'household_not_found', 'Household not found')
  return row
}
...
async function requirePeople(
  ...
  if (result.results.length !== personIds.length) {
    throw new AppError(
      ...
      'Every household member must be an active person in this church',
    )
  }
}
...
householdRoutes.get('/:churchId', async (c) => {
  ...
  if (parsed.data.cursor) {
    const cursor = await c.env.DB.prepare(
      'SELECT name, id FROM households WHERE church_id = ? AND id = ? AND deleted_at IS NULL',
    )
    ...
    if (!cursor)
      throw new AppError(
        422,
        'invalid_cursor',
        'The household cursor is invalid',
      )
    conditions.push(
      '(h.name COLLATE NOCASE > ? COLLATE NOCASE OR (h.name = ? COLLATE NOCASE AND h.id > ?))',
    )
...
householdRoutes.post('/:churchId', async (c) => {
  ...
  await requirePeople(
    c.env.DB,
    churchId,
    parsed.data.members.map((member) => member.personId),
  )
  ...
  return c.json(
    { household: mapHousehold(row, members.get(householdId) ?? []) },
    201,
  )
})
...
householdRoutes.patch('/:churchId/:householdId', async (c) => {
  ...
  if (current.version !== parsed.data.version) {
    throw new AppError(
      409,
      'version_conflict',
      'The household changed after it was loaded',
    )
  }
  const next = {
    name: parsed.data.name ?? current.name,
    street:
      parsed.data.street === undefined ? current.street : parsed.data.street,
...
householdRoutes.put('/:churchId/:householdId/members/:personId', async (c) => {
  ...
  statements.push(
    ...
    c.env.DB.prepare(
      ...
      churchId,
      householdId,
      JSON.stringify({ householdId, personId }),
      now,
      now,
      churchId,
      householdId,
...
householdRoutes.delete(
  '/:churchId/:householdId/members/:personId',
  async (c) => {
    const churchId = c.req.param('churchId')
    const householdId = c.req.param('householdId')
    const personId = c.req.param('personId')
