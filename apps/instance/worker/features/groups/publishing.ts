...
import { type AppEnv, groupSelect, findGroup } from './shared'
...
export const groupPublishingRoutes = new Hono<AppEnv>()

groupPublishingRoutes.get('/:churchId', async (c) => {
...
groupPublishingRoutes.post('/:churchId', async (c) => {
  const churchId = c.req.param('churchId')
...
groupPublishingRoutes.get('/:churchId/:groupId', async (c) => {
  const churchId = c.req.param('churchId')
  await requirePermission(c, churchId, 'groups.write')
  return c.json({
    group: mapGroup(
...
groupPublishingRoutes.patch('/:churchId/:groupId', async (c) => {
  const churchId = c.req.param('churchId')
      ...
      c.env.DB.prepare(
        `
          ...
          WHERE church_id = ? AND id = ? AND deleted_at IS NULL AND version = ?
            AND (? IS NULL OR ? >= (SELECT COUNT(*) FROM group_memberships m
...
groupPublishingRoutes.delete('/:churchId/:groupId', async (c) => {
  const churchId = c.req.param('churchId')
  const groupId = c.req.param('groupId')
  const actor = await requirePermission(c, churchId, 'groups.write')
