...
export async function resolveAccessIdentity(
  ...
  if (!jwks) {
    jwks = createRemoteJWKSet(
      new URL(`${issuer.replace(/\/$/, '')}/cdn-cgi/access/certs`),
    )
  ...
  try {
    ...
    if (
      ...
      throw new AppError(
        401,
        'invalid_identity',
        'A valid Access assertion is required',
      )
    }
    throw error
  }
  ...
  if (!email || !subject) {
    throw new AppError(
      401,
      'invalid_identity',
      'The Access token is missing identity claims',
    )
  }
...
export async function syncCurrentUser(
  ...
  if (existing) {
    ...
    const profileChanged =
      existing.first_name !== identity.firstName ||
      existing.last_name !== identity.lastName
    const lastSeenExpired =
      !existing.last_seen_at ||
      now - existing.last_seen_at >= LAST_SEEN_WRITE_INTERVAL_MS
  ...
  if (userWithEmail?.status === 'active') {
    const concurrentlyLinked = await db
      .prepare(
        'SELECT 1 AS linked FROM auth_identities WHERE provider = ? AND subject = ?',
      )
      .bind(identity.provider, identity.subject)
      .first<{ linked: number }>()
...
export async function requireChurchMembership(
  ...
  if (!membership) {
    throw new AppError(
      403,
      'membership_required',
      'An active church membership is required',
    )
  }
  return user
}
