...
describe('people and household directory', () => {
  it('keeps read, write, and household permissions independently enforced', async () => {
    ...
    expect((await fetchAsLeader('GET', '/api/people/church_demo')).status).toBe(
      200,
    )
    expect(
      (
    ...
    expect(
      (await fetchAsLeader('GET', '/api/households/church_demo')).status,
    ).toBe(403)
  })
  ...
  it('creates, searches, paginates, updates, audits, and soft-deletes people', async () => {
    ...
    expect(updated.person).toMatchObject({
      membershipStatus: 'volunteer',
      version: 2,
    })
    ...
    expect(
      (await request('GET', `/api/people/church_demo/${amy.id}`)).status,
    ).toBe(404)
  })
  ...
  it('manages household membership with optimistic concurrency and search', async () => {
    ...
    const searchedResponse = await request(
      'GET',
      '/api/households/church_demo?query=Jordan',
    )
    const searched = await searchedResponse.json<{ households: Household[] }>()
    expect(searched.households.map((household) => household.id)).toEqual([
    ...
    expect(
      memberUpdated.household.members.find(
        (member) => member.personId === riley.id,
      )?.isPrimary,
    ).toBe(true)
    expect(
      memberUpdated.household.members.find(
        (member) => member.personId === jordan.id,
      )?.isPrimary,
    ).toBe(false)
    ...
    expect(removed.household.members.map((member) => member.personId)).toEqual([
      riley.id,
    ])
    ...
    const auditActions = await env.DB.prepare(
      `
    ...
    const outboxTopics = await env.DB.prepare(
      `
