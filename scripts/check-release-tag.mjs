...
if (!/^v\d+\.\d+\.\d+$/.test(tag ?? '')) {
  throw new Error(
    `Release tags must be stable semantic versions such as ${expectedTag}.`,
  )
}
if (tag !== expectedTag) {
  throw new Error(
    `Tag ${tag} does not match package version ${rootPackage.version}.`,
  )
}
if (rootPackage.version !== instancePackage.version) {
  throw new Error(
    'The root and portable instance application versions must match.',
  )
}
